...
var pJS = function (tag_id, params) {
    ...
    pJS.fn.modes.repulseParticle = function (p) {
        if (
            ...
            if (dist_mouse <= pJS.interactivity.modes.repulse.distance) {
                var opacity_line =
                    pJS.interactivity.modes.repulse.line_linked.opacity -
                    dist_mouse /
                        (1 /
                            pJS.interactivity.modes.repulse.line_linked
                                .opacity) /
                        pJS.interactivity.modes.repulse.distance;
            ...
            var normVec = {
                ...
                repulseFactor = clamp(
                    (1 / repulseRadius) *
                        (-1 * Math.pow(dist_mouse / repulseRadius, 2) + 1.2) *
                        repulseRadius *
                        velocity,
                    0.05,
                    100
            ...
            var d = pJS.interactivity.modes.repulse.distance;
            var pos = {
                x:
                    dist_mouse >= d / 1.1
                        ? p.x - normVec.x * repulseFactor
                        : p.x + normVec.x * repulseFactor,
                y:
                    dist_mouse >= d / 1.1
                        ? p.y - normVec.y * repulseFactor
                        : p.y + normVec.y * repulseFactor,
            };
